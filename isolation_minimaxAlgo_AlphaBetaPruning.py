# Mark Trinquero
# Minimax algorithm with alpha-beta pruning & deepening extensions for optimized gameplay
...
# CUSTOM AI AGENT FOR IMPROVED GAMEPLAY
class CustomPlayer():
    ...
    def move(self, game, legal_moves, time_left):        
        ...
        if game.move_count == 1:
            ...
            if (2,2) in legal_moves:
                best_move = (2,2) # move to the center if availiable
                return best_move
            else:
        ...
        # optimal strategy for early moves
        if game.move_count == 2:
            self.helper = self.move_helper(game)
        # optimal strategy for extended gameplay - uses Alpha Beta Pruning with deepening within time constraints
        if self.helper:
    ...
    # MINIMAX ALGO WITH DEEPENING (as time limit allows)
    ...
    # ALPHA BETA PRUNING FOR ADDITIONAL IMPROVEMENT
    ...
    # ADDING ALPHA BETA DEEPENING FOR ADDITIONAL IMPROVEMENT
...
## GAMEPLAY SETUP AND TESTING
if __name__ == '__main__':
    from isolation import Board

## Ensure custom agent preforms better than a random player
    r = RandomPlayer()
    h = CustomPlayer()
    game = Board(h,r)
    game.play_isolation()
